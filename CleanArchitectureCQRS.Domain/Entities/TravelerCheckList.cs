...
namespace CleanArchitectureCQRS.Domain.Entities
{
    public class TravelerCheckList : AggregateRoot<TravelerCheckListId>
    {
        ...
        {
            var alreadyExists = _items.Any(n => n.Name == item.Name);
        ...
        public TravelerItem GetItem(string name)
        {
            var item = _items.SingleOrDefault(n => n.Name == name);
            
            if(item is null)
            {
                throw new TravelerItemNotFoundException(name);
            }
